...
def grab_headlines():                  
    ...
    ## my_soup = soup.find(class_="view view-most-read view-id-most_read view-display-id-panel_pane_1 view-dom-id-99658157999dd0ac5aa62c2b284dd266")       ## I am finding the "Most Read" part of this html, but I need to make sure my answer still works if data changes!                    
   
    ## my_links = my _soup.find_all('a')        ## I am finding all of the links under the "Most Read" header and putting them into a list

    my_soup = soup.ol('li')             ## I am accessing all the links (starting with 'li' tag) under 'ol' (which is the section "Most Read")

    my_new_list = []                
    for my_link in my_soup:
        my_new_list.append(my_link.string)        ## I am grabbing the headline of each link and appending it to a new list
    return my_new_list
...
def get_umsi_data():
    ...
    umsi_titles = {}
    start_url ='https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastna me_value=&rid=All'
    my_request = requests.get(start_url, headers={'User-Agent': 'SI_CLASS'})
    my_soup = BeautifulSoup(my_request.text, 'html.parser')
    page_count = 0                                          ## initiallizing the page count to the first page (since we have to go all the way to page 12)
    
    while (page_count < 13):                ## I am adding job titles and names that we see on each page
        my_titles = []
        my_names = []
        list_for_name_and_title = []
        my_classes = my_soup.find_all('div', class_='field-item even')         ## finding the class that contains all of the data we need

        for some_item in my_classes:
            if some_item.string != None:                ## I only want names and titles, nothing else. "my_classes" is too big to print so I need to make it smaller by removing   
                list_for_name_and_title.append(some_item.string)    ## I am appending names and titles to a list that contains both.
        
        for item in range(len(list_for_name_and_title)):
            if (item % 2) == 0:                                         ## I want to grab the names from this big list containing both names and titles, and append it to my list of names
               my_names.append(list_for_name_and_title[item])       ## So I am appending only the even items (names) to my_names

            if (item % 2) != 0:                            
               my_titles.append(list_for_name_and_title[item])      ## Now I am appending only the odd items (which happen to be job titles) to my_titles - I know this from printing
            
        for something in range(len(my_names)):
            umsi_titles[my_names[something]] = my_titles[something]             ## adding my_names and my_titles to big dictionary which I will return later

        page_count = page_count + 1
        
        next_url = start_url + "&page=" + str(page_count)                      ## this is to get to the end page URL 
        my_request = requests.get(next_url, headers={'User-Agent': 'SI_CLASS'})  ## need this for every page I go through
        my_soup = BeautifulSoup(my_request.text, 'html.parser')                      ## need this also for every page I loop through
...
def num_students(data):
    num_phd_students = 0                ## initializing count of PhD students
    for value in data.values():
        if "PhD student" == value:              ## identifying the value of the keys in get_umsi_data dict. If the value is "PhD student" then I want to add to my count
            num_phd_students = num_phd_students + 1
    return num_phd_students
